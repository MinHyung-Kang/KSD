...
demo_simple_gaussian <- function(truemu=5, truesd=1, noisemu=0, noisesd=2, n=100){
      ...
      score_q = pryr::partial(scorefunctiongmm, model=truemodel)
...
demo_normal_performance <- function(){
      ...
      for (i in 1:length(noisesd)){
            ...
            noisydata <- data + noise
            result <- KSD(noisydata,score_q=score_q,'rbf',-1.0)
            pval[i] = result$p
      }
...
demo_simple_gamma <- function(trueshape=10, truescale=3, noisemu=5, noisesd=2, n=100){
      ...
      score_q = pryr::partial(gamma_score, shape=trueshape, scale=truescale)
      originalresult <- KSD(data,score_q=score_q,'rbf',-1.0)
      noisyresult <- KSD(noisydata,score_q=score_q,'rbf',-1.0)
...
demo_gmm <- function(){
      ...
      X <- rgmm(model, n=100)
      score_q = pryr::partial(scorefunctiongmm, model=model)
      result <- KSD(X,score_q=score_q, 'rbf',-1.0)

      model2 <- perturbgmm(model)
      score_q = pryr::partial(scorefunctiongmm, model=model2)
      result2 <- KSD(X,score_q=score_q, 'rbf',-1.0)
      ...
      X <- rgmm(model, n=100)
      score_q = pryr::partial(scorefunctiongmm, model=model)
      result <- KSD(X,score_q=score_q, 'rbf',-2.0)

      model2 <- perturbgmm(model)
      score_q = pryr::partial(scorefunctiongmm, model=model2)
      result2 <- KSD(X,score_q=score_q, 'rbf',-2.0)
      print(sprintf('Original p : %f',result$p))
      print(sprintf('Original p : %f',result2$p))
