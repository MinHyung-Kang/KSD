...
#' @param score_q   Score funtion : takes x as input and
...
KSD <- function(x, score_q, kernel='rbf', width=-1, nboot=1000){
      #cleanup()
      set.seed(1)
      ...
      # If score_q = 'gaussian', calculate the score function for standard norm
      if(typeof(score_q) == 'character'){
            if(tolower(score_q) == 'gaussian'){
                  score_q <- function(val){
                        return (-1 * val)
                  }
            }
      }else{
      ...
      if((typeof(h)== 'character' & tolower(h) == 'median') | h == -1){
            ...
            #h = 1
            medInfo = ratio_median_heuristic(x, score_q)
            h = medInfo$h
      }
      ...
      if(is.numeric(score_q) | is.vector(score_q)){
            Sqx = score_q
      }else{
            Sqx = score_q(x)
      }
